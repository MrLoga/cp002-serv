<template>
  <q-page>
    ...
    >
      <q-tab name="SellTxParams" :label="$t('Exchange')" />
      <q-tab name="BuyTxParams" :label="$t('Get')" />
    </q-tabs>

    <q-tab-panels v-model="convertTab" animated>
      <q-tab-panel name="SellTxParams">
...
export default {
  ...
  methods: {
    ...
    senderConvert () {
      let txData = {
        txAction: this.convertTab,
        coinFrom: this.coinSymbol.value,
        coinTo: this.coinSymbolTo.value,
        feeCoinSymbol: 'BIP',
        message: ''
      }
