...
export default {
  ...
  computed: {
    ...mapState({
      contacts: state => state.contacts.contactList
    }),
    ...mapGetters([
      'filterContacts'
      // 'getBlocked'
  ...
  methods: {
    ...
    newContactForm () {
      if (this.newAddress !== '' && this.newName !== '') {
        this.$store.dispatch('NEW_CONTACT', { name: this.newName, address: this.newAddress }).then(contact => {
          // console.log(contact)
    ...
    openContactMenu (contact) {
      ...
      this.$q.dialog({
        ...
        message: this.$t('Would you like to remove') + ` <b>${contact.name}</b> ` + this.$t('from your contact list?'),
        html: true,
        cancel: {
          label: 'Cancel',
          color: 'red',
    ...
    // filterList (val) {
    //   console.log(val)
    //   this.contactsFilter = this.contacts.filter(v => v.name.toLowerCase().indexOf(val.toLowerCase()) > -1)
    //   console.log(this.contactsFilter.length)
    // }
  },
  created () {
