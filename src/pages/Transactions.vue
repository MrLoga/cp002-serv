<template>
  <q-page padding>
    <q-checkbox v-model="showMultisend" label="Show Multisend" />
    <div v-if="transactions">
      <q-list class="transactions" separator v-for="(transactionList, index) in transactions" :key="index">
        <div class="text-grey text-h6 q-mb-xs q-mt-md" v-show="checkOnlyMultisend(transactionList) || showMultisend">{{ index}}</div>
...
export default {
  ...
  methods: {
    ...
    async loadTransactions (address, page) {
      const data = await this.$store.dispatch('FETCH_TRANSACTION')
