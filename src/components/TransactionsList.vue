    ...
    async loadTransactions () {
      ...
      const updateData = data.data.map(item => {
        ...
        if (tmpObj.type === 'SEND') {
          ...
          tmpObj.symbol = '+'
          tmpObj.title = `<span class='text-red'>- ${ prettyNumber(item.data.value_to_sell, 2) } ${ item.data.coin_to_sell }</span>`
          tmpObj.value = prettyNumber(item.data.value_to_buy, 2)
          ...
          if (item.from === this.address) {
            ...
            const sendData = item.data.list.find(msend => msend.to === this.address)
            tmpObj.value = prettyNumber(sendData.value, 2)
